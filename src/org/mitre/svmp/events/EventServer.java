...
public class EventServer extends BaseServer {
    ...
    private final MotionEvent.PointerCoords translateCoords(float X, float Y) {
    ...
    private final void handleTouchNew(final SVMPProtocol.TouchEvent event) {
        // Maintain Downtime
        final long now = SystemClock.uptimeMillis();
        if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_DOWN) {
            Log.d(TAG, "ACTION_DOWN at " + now + " / " + event.getEventTime());
            lastDownTime = now;
            lastDownTimeClient = event.getDownTime();
        }

        if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP) {
            Log.d(TAG, "ACTION_UP at " + now + " / " + event.getEventTime());
        }

        if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_POINTER_UP) {
            int index = (event.getAction() & MotionEvent.ACTION_POINTER_INDEX_MASK) >> MotionEvent.ACTION_POINTER_INDEX_SHIFT;
            Log.d(TAG, "ACTION_POINTER_UP(" + index + ") at " + now + " / " + event.getEventTime());
        }

        if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_POINTER_DOWN) {
            int index = (event.getAction() & MotionEvent.ACTION_POINTER_INDEX_MASK) >> MotionEvent.ACTION_POINTER_INDEX_SHIFT;
            Log.d(TAG, "ACTION_POINTER_DOWN(" + index + ") at " + now + " / " + event.getEventTime());
        }

        //if (lastDownTimeClient != event.getDownTime()) {
        //    lastDownTimeClient = event.getDownTime();
        //    lastDownTime = now;
        //}
        ...
        MotionEvent me = MotionEvent.obtain(lastDownTime,                           // downTime
                                            offsetEventTime(event.getEventTime()),  // eventTime
    ...
    private final long offsetEventTime(long clientTime) {
        return clientTime - lastDownTimeClient + lastDownTime;
    }

    private final void injectTouch(MotionEvent me) {
        try {
